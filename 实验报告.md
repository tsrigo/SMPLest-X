...
# 带骨骼可视化的推理
bash scripts/inference.sh smplest_x_h test_image.jpg 30 true
```

**参数说明**:
- 参数1: 模型名称 (`smplest_x_h`)
- 参数2: 输入文件名 (`test_image.jpg`)
- 参数3: 帧率 (默认: `30`)
- 参数4: 骨骼可视化开关 (可选值: `true`/`1`/`skeleton`/`false`, 默认: `false`)
...
# 带骨骼可视化的推理
bash scripts/inference.sh smplest_x_h test_video.mp4 30 skeleton
```
...
### 6.1 骨骼可视化功能 (SHOW_SKELETON)

SMPLest-X 支持在输出结果中叠加人体骨骼关键点，便于更直观地观察姿态估计效果。

**功能说明**:
- 在渲染的 SMPL-X 网格模型上叠加 2D 骨骼关键点
- 使用线条连接关键点，形成完整的人体骨架
- 适用于图片和视频推理

**使用方法**:

启用骨骼可视化需要在推理脚本的第 4 个参数中指定，支持以下值：
- `true`: 启用骨骼可视化
- `1`: 启用骨骼可视化
- `skeleton`: 启用骨骼可视化
- `false` 或不指定: 禁用骨骼可视化（默认）
...
# 使用数字 1 启用
bash scripts/inference.sh smplest_x_h test_image.jpg 30 1

# 不启用骨骼可视化（默认）
bash scripts/inference.sh smplest_x_h test_image.jpg 30
bash scripts/inference.sh smplest_x_h test_image.jpg 30 false
```

**技术实现**:
- 脚本参数: `scripts/inference.sh` 第 4 个参数
- Python 参数: `main/inference.py --show_skeleton`
- 渲染函数: `utils/visualization_utils.py` 中的 `render_mesh()` 函数
- 关键点来源: SMPL-X 模型的 2D 投影关键点
